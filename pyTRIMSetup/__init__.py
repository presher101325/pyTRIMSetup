...
from . import _AtomicInfo as atom   # Conatins info on each atom, as found in SRIM
    ...
    def output(self, filepath, options=None, overwrite=False, warn=True, split=False, numlayers=30):
        ...
        if _platform == "linux" or _platform == "linux2":
           ...
           le = '\n'
            ...
            print("WARNING: Can't determine platform - assuming Windows system & corresponding line-endings.")
            le = '\n'
        ...
        for Is,stacks in enumerate(allstacks):
            ...
            print("Writing to file: %s" %(filepath))
            f = open(filepath,'w')
            f.write('==> SRIM-2013.00.  Generated by pyTRIMSetup (Demis D. John 2015).' + le) # 1st comment line
            #f.write(ts[1])  # comment line: Ion: Z1...
            ...
            f.write('"%s"' %(options['Title'])  +  tab + str(stacks.get_numElements()) + tab + str(len(stacks)) + le)
